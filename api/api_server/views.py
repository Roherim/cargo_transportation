...
def new_delivery(request):
    ...
    try:
        ...
        print("Received data for new delivery:", data)  # Логируем полученные данные
        
        # Проверяем наличие всех обязательных полей
        required_fields = ['transport_model', 'transport_number', 'pickup_address', 
                         'delivery_address', 'packaging_type', 'services', 
                         'departure_date', 'departure_time', 
                         'arrival_date', 'arrival_time', 'travel_time',
                         'cargo_type']
        ...
        try:
            # Получаем первый активный статус
            default_status = DeliveryStatus.objects.filter(is_active=True).first()
            if not default_status:
                return JsonResponse({
                    'error': 'No active delivery statuses found'
                }, status=400)
            ...
            delivery = Delivery.objects.create(
                number=number,  # Добавляем сгенерированный номер
                ...
                created_by=request.user  # Добавляем created_by
            )
            ...
            print("Delivery created successfully:", delivery)  # Логируем успешное создание
            
            return JsonResponse({
                'id': delivery.id,
                'number': delivery.number,  # Возвращаем номер в ответе
                'message': 'Delivery created successfully'
            })
        except Exception as e:
            print("Error creating delivery:", str(e))  # Логируем ошибку создания
            return JsonResponse({
                'error': f'Error creating delivery: {str(e)}'
            }, status=400)
            
    except json.JSONDecodeError as e:
        print("JSON decode error:", str(e))  # Логируем ошибку декодирования JSON
        return JsonResponse({
...
def update_delivery(request, delivery_id):
    ...
    try:
        data = json.loads(request.body)
        print("Received update data:", data)  # Логируем полученные данные
        ...
        print("Delivery not found:", delivery_id)  # Логируем отсутствие доставки
        return JsonResponse({
